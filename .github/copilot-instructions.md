...
## Developer workflows
- Local dev: backend in [backend/main.py](../backend/main.py) (`python main.py`) per [README.md](../README.md).
...
- Docker compose serves backend at http://localhost:5001 (see [README.md](../README.md)).
...
- **LLM change requirement:** whenever any fixture effect is added, removed, renamed, or its parameter contract is changed in backend models/fixtures data, update protocol documentation and active client implementation details in the same change.
...
- Control clients should use backend WebSocket messages (`playback`, `timecode`, `seek`) as the canonical playback control interface.
