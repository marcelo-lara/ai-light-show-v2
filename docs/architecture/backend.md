...
- `sweep` moving-head cues compute a dark pre-roll from the last known pan/tilt position to `start_POI`, using each fixture template's `physical_movement.pan_full_travel_seconds` and `physical_movement.tilt_full_travel_seconds` values plus an extra `100 ms` safety pre-roll. The renderer starts the cue early by that offset, moves the head to `start_POI`, holds there dark for `100 ms`, then uses per-leg cubic easing: ease-out into `subject_POI`, ease-in away from it, and a mirrored dimmer envelope controlled by `dimmer_easing` (`0` starts fading immediately at visible start, `1` delays until almost at the subject). When the fixture is on the subject POI, output intensity reaches `max_dim`.
