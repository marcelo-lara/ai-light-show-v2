...
export function List(options: ListOptions): HTMLElement {
	...
	row.className = `c-list__row ${options.className ?? ""}`.trim();
	...
	content.className = `c-list__content ${options.contentClassName ?? ""}`.trim();
	...
	actions.className = `c-list__actions ${options.actionsClassName ?? ""}`.trim();
