...
- Untimed POI-aware fixture actions such as moving, orbiting, or sweeping a resolved fixture use the staged LLM tool loop: fixtures first, then POIs, then cursor when time is omitted, then a grounded cue-add proposal using exact fixture ids and POI ids.
