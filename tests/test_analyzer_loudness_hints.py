...
def _load_hints_module():
    module_path = Path(__file__).resolve().parents[1] / "analyzer" / "src" / "essentia_analysis" / "hints.py"
    spec = importlib.util.spec_from_file_location("essentia_hints", module_path)
...
def test_build_loudness_hints_merges_cross_stem_events():
    module = _load_hints_module()
    payload = module.build_loudness_hints(
        {
...
def test_build_loudness_hints_adds_section_sustain_and_song_fallback():
    module = _load_hints_module()
    payload = module.build_loudness_hints(
    ...
    fallback = module.build_loudness_hints(
