...
def handle(
    ...
    payload = data or {}
    target_value = payload.get(dim_key, payload.get("dim", payload.get("dimmer", payload.get("intensity", 255))))
    target_dim = self._clamp_byte(target_value)
    ...
    current_dim = int(round(start_dim + ((target_dim - start_dim) * progress)))
