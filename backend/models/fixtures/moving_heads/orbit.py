...
from .orbit_helpers import spiral_orbit_position
...
def handle(self, universe: bytearray, frame_index: int, start_frame: int, end_frame: int, fps: int, data: Dict[str, Any], render_state: Dict[str, Any]) -> None:
    # Orbit around subject_POI, then spiral into it.
    ...
    preroll_frames = max(0, int(payload.get("__orbit_preroll_frames") or 0))
    ...
    if "orbit_initial_pan_u16" not in render_state or "orbit_initial_tilt_u16" not in render_state:
        render_state["orbit_initial_pan_u16"] = int(self._read_axis_u16_from_universe(universe, "pan") or 0)
        render_state["orbit_initial_tilt_u16"] = int(self._read_axis_u16_from_universe(universe, "tilt") or 0)

    if "orbit_last_pan_u16" not in render_state or "orbit_last_tilt_u16" not in render_state:
        render_state["orbit_last_pan_u16"] = int(render_state.get("orbit_initial_pan_u16", 0))
        render_state["orbit_last_tilt_u16"] = int(render_state.get("orbit_initial_tilt_u16", 0))

    intensity_key = find_intensity_channel_key(self)
    if intensity_key and "orbit_initial_intensity" not in render_state:
        render_state["orbit_initial_intensity"] = int(universe[self.absolute_channels[intensity_key] - 1])

    last_pan_u16 = int(render_state.get("orbit_last_pan_u16", 0))
    last_tilt_u16 = int(render_state.get("orbit_last_tilt_u16", 0))
    ...
    if frame_index < start_frame + move_frames:
        ...
        target_pan_u16 = int(round(int(render_state.get("orbit_initial_pan_u16", 0)) + ((int(start_pan_u16) - int(render_state.get("orbit_initial_pan_u16", 0))) * move_progress)))
        target_tilt_u16 = int(round(int(render_state.get("orbit_initial_tilt_u16", 0)) + ((int(start_tilt_u16) - int(render_state.get("orbit_initial_tilt_u16", 0))) * move_progress)))
        ...
        tilt_value = self._clamp_u16(limit_axis_step(last_tilt_u16, target_tilt_u16, max_tilt_step))
        if intensity_key:
            self._write_channel(universe, intensity_key, int(render_state.get("orbit_initial_intensity", 0)))
    else:
    ...
    render_state["orbit_last_pan_u16"] = int(pan_value)
    render_state["orbit_last_tilt_u16"] = int(tilt_value)