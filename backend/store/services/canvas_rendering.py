...
from store.services.canvas_render_core import estimate_sweep_preroll_seconds, iter_cues_for_render, render_entry_into_universe
...
def render_preview_canvas(
    *,
    ...
    if effect == "sweep":
        ...
        preroll_seconds = estimate_sweep_preroll_seconds(fixture, preview_data, last_position)
        preroll_frames = max(0, int(round(preroll_seconds * fps)))
        if preroll_frames > 0:
            preview_data["__sweep_preroll_frames"] = preroll_frames
