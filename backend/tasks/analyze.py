...
@celery_app.task(bind=True, queue='analyzer')
def analyze_song(self, song_path: str, device: str = 'auto', out_dir: str = None, temp_dir: str = None, overwrite: bool = False):
    ...
    try:
        ...
        import_error = None
        # Support both package names: 'analyzer.song_analyzer' (tests) and 'song_analyzer' (analyzer image)
        for pkg in ('analyzer.song_analyzer', 'song_analyzer'):
            try:
                cfg_mod = __import__(f"{pkg}.config", fromlist=["AnalysisConfig"])
                pipeline_mod = __import__(f"{pkg}.pipeline", fromlist=["AnalysisPipeline"])
                AnalysisConfig = cfg_mod.AnalysisConfig
                AnalysisPipeline = pipeline_mod.AnalysisPipeline
                analyzer_available = True
                import logging
                logging.getLogger('tasks.analyze').info(f"Imported analyzer package: {pkg}")
                # Also print to stdout so this shows in container logs reliably
                print(f"Imported analyzer package: {pkg}")
                break
            except Exception as import_exc:
                import_error = str(import_exc)
                import logging
                logging.getLogger('tasks.analyze').debug(f"Failed importing {pkg}: {import_error}")

        if not analyzer_available:
            # Log import failure for diagnostics
            import logging
            logging.getLogger('tasks.analyze').warning(f"Analyzer modules not available, import error: {import_error}")
