...
- `sweep` schedules a dark pre-roll based on the fixture's previous pan/tilt position and the template's `physical_movement.pan_full_travel_seconds` / `physical_movement.tilt_full_travel_seconds` metadata, plus an extra `100 ms` safety pre-roll. During that pre-roll the head moves to `start_POI`, holds there dark for `100 ms`, then runs cubic ease-out into `subject_POI`, cubic ease-in away from the subject, and an independent mirrored dimmer envelope. `dimmer_easing` is a normalized `0..1` control for how late fade-in begins before the subject, and `max_dim` is guaranteed when pan/tilt land on the subject POI.
