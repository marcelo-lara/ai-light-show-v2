...
async def apply_chaser(manager, payload: Dict[str, Any]) -> bool:
    ...
    if not chaser_name:
        await manager.broadcast_event("error", "chaser_apply_failed", {"reason": "missing_chaser_name"})
        return False
    ...
    result = await manager.state_manager.apply_chaser(chaser_name, start_time_ms_f, repetitions_i)
