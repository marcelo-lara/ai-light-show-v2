...
TASK_DEFINITIONS = [
    _task(
        ...
        notes=["Creates only song_name, song_path, and artifacts."],
    ),
    ...
    _task(FIND_BEATS_TASK, requires=["source song path", "info.json root"], produces=["beats.json", "info.json beat metadata"], notes=["Uses Moises beat source when compatible Moises chord data exists."]),
    ...
    _task(FIND_SONG_FEATURES_TASK, requires=["info.json", "beats.json", "essentia mix artifacts"], produces=["features.json", "info.json feature summary"], notes=["Also uses sections.json and hints.json when present."]),
    _task(FIND_CHORDS_TASK, requires=["beats.json"], produces=["beat-aligned chord metadata", "info.json musical structure metadata"], notes=["Optional enrichment step."]),
    _task(FIND_SECTIONS_TASK, requires=["beats.json"], produces=["sections.json", "info.json musical structure metadata"], notes=["Used when analyzer must infer sections or when Moises segments are unavailable."]),
    _task(IMPORT_MOISES_TASK, requires=["moises/chords.json"], produces=["beats.json", "optional sections.json", "info.json beat metadata"], notes=["Moises files are external source-of-truth inputs and are never overwritten or deleted."]),
